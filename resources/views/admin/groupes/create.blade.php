...
@section('header-title', 'Nouveau Groupe')
...
<div class="py-6">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-5 flex items-center gap-2 text-sm text-slate-400">
            <a href="{{ route('admin.groupes.index') }}" class="hover:text-primary-600 transition">Groupes</a>
            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <span class="text-slate-600 font-medium">Nouveau groupe</span>
        </div>
        ...
        <div class="bg-white shadow rounded-2xl border border-slate-100 overflow-hidden">
            ...
            <div class="px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-violet-50">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-base font-bold text-slate-800">Créer un nouveau groupe</h2>
                        <p class="text-xs text-slate-500 mt-0.5">Créez une division d'étudiants rattachée à une filière</p>
                    </div>
                </div>
            </div>

            {{-- Form --}}
            <form action="{{ route('admin.groupes.store') }}" method="POST" class="p-6 space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="filiere_id" class="block text-sm font-semibold text-slate-700 mb-1.5">
                            Filière d'études <span class="text-rose-500">*</span>
                        </label>
                    ...
                    <div>
                        <label for="capacite" class="block text-sm font-semibold text-slate-700 mb-1.5">
                            Capacité maximale d'étudiants <span class="text-rose-500">*</span>
                        </label>
                        <input type="number" name="capacite" id="capacite" value="{{ old('capacite', '30') }}" required
