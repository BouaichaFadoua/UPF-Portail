...
@section('header-title', 'Modifier une Réservation')
...
<div class="py-6">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow rounded-2xl border border-slate-100 overflow-hidden">
            <form action="{{ route('admin.reservations.update', $reservation) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="professeur_id" required class="w-full text-sm border border-slate-200 rounded-xl px-4 py-2.5 bg-white">
                        @foreach($professeurs as $prof)
                            <option value="{{ $prof->id }}" {{ old('professeur_id', $reservation->professeur_id) == $prof->id ? 'selected' : '' }}>{{ $prof->user->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="date" name="date" value="{{ old('date', $reservation->date->format('Y-m-d')) }}" required class="w-full text-sm border border-slate-200 rounded-xl px-4 py-2.5">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1.5">Heure début <span class="text-rose-500">*</span></label>
                        <input type="time" name="heure_debut" value="{{ old('heure_debut', substr($reservation->heure_debut, 0, 5)) }}" required class="w-full text-sm border border-slate-200 rounded-xl px-4 py-2.5">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1.5">Heure fin <span class="text-rose-500">*</span></label>
                        <input type="time" name="heure_fin" value="{{ old('heure_fin', substr($reservation->heure_fin, 0, 5)) }}" required class="w-full text-sm border border-slate-200 rounded-xl px-4 py-2.5">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5">Motif <span class="text-rose-500">*</span></label>
                    <input type="text" name="motif" value="{{ old('motif', $reservation->motif) }}" required class="w-full text-sm border border-slate-200 rounded-xl px-4 py-2.5">
                </div>
                <div>
                    ...
                    <select name="statut" class="w-full text-sm border border-slate-200 rounded-xl px-4 py-2.5 bg-white">
                        @foreach(['en_attente', 'approuvee', 'refusee', 'annulee'] as $s)
                            <option value="{{ $s }}" {{ old('statut', $reservation->statut) === $s ? 'selected' : '' }}>{{ ucfirst($s) }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="pt-4 flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-6 py-2.5 text-sm font-semibold text-white bg-primary-600 rounded-xl hover:bg-primary-500">Enregistrer</button>
                </div>
            </form>
        </div>
    </div>
